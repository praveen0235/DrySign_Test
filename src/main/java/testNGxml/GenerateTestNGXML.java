...
public class GenerateTestNGXML 
{
	...
	private static GenerateTestNGXML xmlobj;
	Map<String,String> getParamAttr=new HashMap<String,String>();
	 ...
	 public static GenerateTestNGXML getInstance()
	 {
	        if(xmlobj == null){
	        	xmlobj = new GenerateTestNGXML();
	        }
	        return xmlobj;
	 }
			...
			String platformtype, Properties prop, String className)
	{
		...
		if(platformtype.equals("Android_Web")||platformtype.equals("Android_App")||platformtype.equals("OS X")||platformtype.equals("Windows")||platformtype.equals("Mobile"))
		{
			...
			browser_br.setValue(paramAttr.get("browser"));
			...
			Parameter browser_version = new Parameter();
			browser_version.setName("browser_version");
			browser_version.setValue(paramAttr.get("browser_version"));
			
			Parameter resolution = new Parameter();
			resolution.setName("resolution");
			resolution.setValue(paramAttr.get("resolution"));
			
			
			
			if(browser_version.getValue()!=null && !browser_version.getValue().isEmpty())
			{
			myparam.add(browser_version);
			}
			
			if(resolution.getValue()!=null && !resolution.getValue().isEmpty())
			{
			myparam.add(resolution);
			}
			
			
			if(name_br.getValue()!=null && !name_br.getValue().isEmpty())
			{
			myparam.add(name_br);
			}
			
			if(browserstack_debug_br.getValue()!=null && !browserstack_debug_br.getValue().isEmpty())
			{
			myparam.add(browserstack_debug_br);
			}
			
			if(browserstack_local_br.getValue()!=null && !browserstack_local_br.getValue().isEmpty())
			{
			myparam.add(browserstack_local_br);
			}
			
			if(browserstack_networkLogs_br.getValue()!=null && !browserstack_networkLogs_br.getValue().isEmpty())
			{
			myparam.add(browserstack_networkLogs_br);
			}
			
			if(SuiteName_br.getValue()!=null && !SuiteName_br.getValue().isEmpty())
			{
			myparam.add(SuiteName_br);
			}
			
			if(platformType_br.getValue()!=null && !platformType_br.getValue().isEmpty())
			{
			myparam.add(platformType_br);
			}
			
			if(browser_br.getValue()!=null && !browser_br.getValue().isEmpty())
			{
			myparam.add(browser_br);
			}
			
			if(nodeparam.getValue()!=null && !nodeparam.getValue().isEmpty())
			{
			myparam.add(nodeparam);
			}
			
			if(browparam.getValue()!=null && !browparam.getValue().isEmpty())
			{
			myparam.add(browparam);
			}
			
			if(platparam.getValue()!=null && !platparam.getValue().isEmpty())
			{
			myparam.add(platparam);
			}
			
			if(tsuiteparam.getValue()!=null && !tsuiteparam.getValue().isEmpty())
			{
			myparam.add(tsuiteparam);
			}
			
			if(sceanrios.getValue()!=null && !sceanrios.getValue().isEmpty())
			{
			...
			if(AccessKey.getValue()!=null && !AccessKey.getValue().isEmpty())
			{
	...
	public ArrayList<String> getKey(Xls_Reader suiteXLS) 
	{
		String os_version = null, device = null, real_mobile = null, project = null,build = null,name=null,
				browserstack_local=null,browserstack_debug= null,browserstack_networkLogs=null,UserName=null,
						SuiteName=null,browser=null,platformType=null, AccessKey=null, browser_version=null, resolution=null;
		...
		try 
		{
			for (int i = 2; i <=suiteXLS.getRowCount("TestConfig"); i++) 
			{
				...
				keyList.add(browser_version);
				
				resolution="resolution";
				keyList.add(resolution);
	...
	public void createXML(Xls_Reader suiteXLS, Properties prop) 
	{
		...
		for (int i = 2; i <=suiteXLS.getRowCount("TestConfig"); i++) 
		{
				...
				String browser_version = suiteXLS.getCellData("TestConfig", "browser_version", i);
				String resolution = suiteXLS.getCellData("TestConfig", "resolution", i);
				...
				getParamAttr.put(key.get(14), browser_version);
				getParamAttr.put(key.get(15), resolution);
								
										
				if(tSuiteRunMode.equalsIgnoreCase("Yes"))
				{
					myTests.add(createTesttag("test" + i, getParamAttr, platformType, prop,"runner.Runner"));
				}
			}
